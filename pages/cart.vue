<template>
    <div class="cart-list gap-6">
        <div class="cart-item grid grid-cols-6 rounded" v-for="item in cartItems" :key="item.id">
...
const cartItems = computed(() => {
    return Object.values(cartStore.items).map(cartItem => {
        for (const categoryKey in data) {
            const foundItem = data[categoryKey].find(item => item.id === cartItem.id);
            if (foundItem) {
                return {
                    ...foundItem,
